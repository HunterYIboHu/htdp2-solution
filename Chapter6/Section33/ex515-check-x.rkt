...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex515-check-x) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; definitions
; A Lam is one of:
; - a Symbol
...
;; test
(check-expect (undeclareds '((λ (x) x) x))
              '((λ (x) x) *undeclared))


;; Questions
;; Q1: Does undeclareds work properly on your expression?
;; A1: Yes. Because the procession of an application's argument
;; do not renew the accumulator declared.
