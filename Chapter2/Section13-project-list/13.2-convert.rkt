...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname 13.2-convert) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define 1-music-s (create-track "Ascendance" "Lindsey Stirling" "Shatter Me"
                            266000 8 1-add 45 1-play))
(define 2-music-s (create-track "Master of Tides" "Lindsey Stirling" "Shatter Me"
                            261000 11 2-add 60 2-play))
(define 3-music-s (create-track "Animals" "Maroon5" "Animals"
...
;; main functions
; LAssoc -> Track
; convert a la into the corresponding Track.

(check-expect (lassoc->track 1-music) 1-music-s)
(check-expect (lassoc->track 2-music) 2-music-s)

(define (lassoc->track la)
  (create-track (extract "name" la)
                (extract "artist" la)
                (extract "album" la)
                (extract "time" la)
                (extract "track#" la)
...
; Track -> LAssoc
; convert a given t into the corresponding LAssoc.

(check-expect (track->lassoc 1-music-s) 1-music)
(check-expect (track->lassoc 2-music-s) 2-music)

(define (track->lassoc t)
  (list (create-name (track-name t))
        (create-artist (track-artist t))
        (create-album (track-album t))
        (create-time (track-time t))
        (create-track# (track-track# t))
        (create-added (track-added t))
        (create-play# (track-play# t))
        (create-played (track-played t))))


;; auxiliary functions
; String LAssoc -> BSDN
; extract the second item of specific association

(check-expect (extract "name" 1-music) "Ascendance")
(check-expect (extract "artist" 1-music) "Lindsey Stirling")
