...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex406-hoist-row-project) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (project db labels)
  (local ((define schema (db-schema db))
          ...
          (define (row-filter row names)
            (cond [(empty? row) '()]
                  [else (local ((define tail (row-filter (rest row)
                                                         (rest names))))
