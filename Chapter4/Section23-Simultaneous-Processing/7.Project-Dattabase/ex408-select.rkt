...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex408-select) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; data difinitions
...
; produces a list of rows that satisfy the given predicate,
; projected down to the given set of labels

(check-expect (db-content (select school-db '("Name")
                                  (lambda (row)
                                    (string=? (first row) "Alice"))))
              '(("Alice")))
(check-expect (db-content (select school-db
              ...
              '(("Bob" 25 #false)
                ("Carol" 30 #true)))
(check-expect (db-content (select presence-db
                                  '("Description")
                                  (lambda (row)
                                    (boolean=? (first row) #true))))
              '(("presence")))
