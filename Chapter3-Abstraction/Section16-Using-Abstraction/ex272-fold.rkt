...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname ex272-fold) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor mixed-fraction #f #t none #f () #f)))
(require 2htdp/image)
...
(define (append-from-fold former latter)
  (local (; [List-of X] X -> [List-of X]
    ...
    (foldr add-at-end former (reverse latter))))
...
(define (append-from-foldl former latter)
  (local (; [List-of X] X -> [List-of X]
          ...
          (define (add-at-end x l)
            (reverse (cons x (reverse l)))))
    (foldl add-at-end former latter)))
