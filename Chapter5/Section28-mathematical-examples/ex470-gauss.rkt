...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex470-gauss) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; definitions
...
;; functions
; SOE -> Solution
; produces the solution of soe.

(check-expect (gauss M) S)
(check-expect (gauss M-4) S-2)
(check-expect (gauss '((1 2))) '(2))
(check-expect (gauss '((3 9 21) (-3 -8 -19))) '(1 2))

(check-error (gauss '()))

(define (gauss soe)
  (solve-v2 (triangulate soe)))


; SOE -> Solution
; produces the solution of e, which is an equation after
; triangulating.

(check-expect (solve (triangulate M)) S)
(check-expect (solve (triangulate M-4)) S-2)
...
(define (solve e)
  (cond [(empty? e) (error "e shall not be empty list.")]
        [(empty? (rest e)) `(,(solve-one (first e) '()))]
        [else (local ((define rest-solution (solve (rest e))))
                (cons (solve-one (first e) rest-solution)
                      rest-solution))]))
...
(check-expect (solve-v2 (triangulate M)) S)
(check-expect (solve-v2 (triangulate M-4)) S-2)
(check-expect (solve-v2 '((1 2))) '(2))
(check-expect (solve-v2 '((3 9 21) (1 2))) '(1 2))

(check-error (solve-v2 '()))

(define (solve-v2 soe)
  (if (empty? soe)
      (error "soe shall not be empty list.")
      (foldr (lambda (e lov) (cons (solve-one e lov)
                                   lov))
             '()
             soe)))
...
; produces the value of the first variable of e, the other variables'
; value is given in lov.
; assume the given lov's length is (- (length e) 2);
; assume the first number of equation is not zero.

(check-expect (solve-one '(2 3 1 10) '(1 3)) 2)
(check-expect (solve-one '(10 5 6 20) '(0 0)) 2)
(check-expect (solve-one '(10 5 2 20) '(2 5)) 0)
(check-expect (solve-one '(2 2 3 10) '(1 2)) 1)
(check-expect (solve-one '(3 9 21) '(2)) 1)
(check-expect (solve-one '(1 2) '()) 2)

(define (solve-one e lov)
  (/ (- (rhs e)
        (foldr + 0 (map (lambda (coefficient value)
                          (* coefficient value))
                        (rest (lhs e)) lov)))
     (first e)))
