...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex429-quick-sort-V5-use-filter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; check functions
(define check-sort
...
(check-expect (quick-sort<.v5 '(1 6 2 99 4 23))
              '(1 2 4 6 23 99))
(check-expect (quick-sort<.v5 '(10 40 30 20))
              '(10 20 30 40))
(check-expect (quick-sort<.v5 '()) '())
(check-expect
 (quick-sort<.v5 '(84 55 65 31 61 40 19 66 74 63 13 49 26 44 43 27 34
...
(check-satisfied (quick-sort<.v5 check-list) check-sort)
    ...
    (cond [(empty? alon) '()]
          ...
          [else (local ((define pivot (first alon)))
                  (append (quick-sort<.v5 (smallers.v2 alon pivot))
                          (list pivot)
                          (quick-sort<.v5 (largers.v2 (rest alon) pivot))))])))
...
(check-expect (smallers.v2 '(1 2 5 77 3 4) 4) '(1 2 3))
(check-expect (smallers.v2 '(1 2 5 77 3 4) 0) '())
(check-expect (smallers.v2 '() 0) '())

(define (smallers.v2 alon pivot)
