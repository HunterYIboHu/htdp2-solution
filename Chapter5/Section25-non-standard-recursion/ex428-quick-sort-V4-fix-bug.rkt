...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex428-quick-sort-V4-fix-bug) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; check functions
(define check-sort
  (lambda (l) (equal? l (sort l <))))
(define check-list (map random (make-list 100 100)))
...
(check-expect (quick-sort<.v4 '(1 6 2 99 4 23))
              '(1 2 4 6 23 99))
(check-expect (quick-sort<.v4 '(10 40 30 20))
              '(10 20 30 40))
(check-expect (quick-sort<.v4 '()) '())
(check-expect
 (quick-sort<.v4 '(84 55 65 31 61 40 19 66 74 63 13 49 26 44 43 27 34
...
(check-satisfied (quick-sort<.v4 check-list) check-sort)
...
(check-expect (largers.v2 '(1 2 5 77 3 4) 4) '(5 77 4))
(check-expect (largers.v2 '(1 2 5 77 3 4) 78) '())
(check-expect (largers.v2 '() 0) '())

(define (largers.v2 alon pivot)
  (filter (lambda (num) (>= num pivot)) alon))
...
;; Questions
;; Q1: If the input to quick-sort< contains the same number several times,
;; the algorithm returns a list that is strictly shorter than the input. Why?
;;
;; A1: Because the neither largers nor smallers contains the equal numbers of qivot. 
